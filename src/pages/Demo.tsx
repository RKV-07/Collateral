...
import { calculateOptimizer } from "../utils";
import { api, ApiError, PortfolioDTO, HoldingInput, PriceInfo } from "../api";
...
export default function Demo() {
  ...
  const mode: "preset" | "live" = user ? "live" : "preset";

  const [selectedPresetKey, setSelectedPresetKey] = useState<string>("breached");

  // Portfolio / prices (live mode)
  ...
  // Holdings (shared state, seeded per mode)
  ...
  const refreshPrices = useCallback(
    async (symbols: string[]) => {
      if (mode === "preset" || symbols.length === 0) return;
  ...
  const loadPortfolio = useCallback(async () => {
    try {
      ...
      for (const h of data.portfolio.holdings) {
  ...
  // (Re)initialize on mode / preset change
  useEffect(() => {
    if (mode === "live") {
      setLoadingPortfolio(true);
      void loadPortfolio();
    } else {
      const pre = INITIAL_PRESETS[selectedPresetKey].snapshot;
      setCash(pre.cash);
      setLoanBalance(pre.loan_balance);
      setMaintenanceLimit(pre.maintenance_ltv_limit);
      setHoldings(pre.holdings.map((h) => ({ ...h })));
      setPrices({});
      setPortfolioError(null);
      setLoadingPortfolio(false);
    }
  ...
  const persistAccountProps = useCallback(
    (next: { cash?: number; loanBalance?: number; maintenanceLtvLimit?: number }) => {
  ...
  // Approve & Execute — simulated locally in preset mode, persisted in live mode
    ...
    if (mode === "preset") {
      const updatedHoldings = holdings
        .map((lot) => {
          ...
          if (proposed) return { ...lot, quantity: Math.max(0, lot.quantity - proposed.quantity) };
          return lot;
        })
      ...
      setCashNeed(0);
      setMarketDropPct(0);
      setToast("Demo mode: trade simulated locally. Sign in to execute against your real portfolio.");
      return;
    }
  ...
  }, [proposal, holdings, loanBalance, cashNeed, maintenanceLimit, mode, loadPortfolio]);

  // Holding CRUD — local in preset mode, persisted in live mode
  const handleAddHolding = useCallback(
    async (data: HoldingInput) => {
      setIsSaving(true);
      try {
        if (mode === "preset") {
          const id = `lot_demo_${Date.now()}`;
          setHoldings((prev) => [
            ...prev,
            {
              id,
              ...
              acquired_date: data.acquiredAt,
              current_price: data.costBasis,
            },
          ]);
          return;
        }
            ...
            prev.map((l) =>
              l.id === lotId
                ? {
                    ...
                    cost_basis: patch.costBasis ?? l.cost_basis,
                    acquired_date: patch.acquiredAt ?? l.acquired_date,
                  }
                : l
            )
          );
  ...
  const handleDeleteLot = useCallback(
    async (holdingId: string) => {
      ...
      try {
        if (mode === "preset") {
          setHoldings((prev) => prev.filter((l) => l.id !== holdingId));
          return;
        }
  ...
  return (
    <div className="min-h-screen bg-surface text-[#E0E0E0] pb-16 font-sans selection:bg-white/20 selection:text-white">
      {/* Demo banner */}
      <div
        className={`px-6 py-2.5 text-xs font-mono flex items-center justify-center gap-2 border-b ${
          mode === "preset"
      ...
      >
        {mode === "preset" ? (
          <>
            ...
            <span>
              Demo mode — exploring <strong className="font-bold">rough sample data</strong>. Sign in to load your own
              portfolio.
            </span>
          ...
          <>
            <LogIn size={13} />
            <span>
              Signed in as <strong className="font-bold">{user?.email}</strong> — running on your real portfolio.{" "}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-white/40">
                {mode === "preset" ? "Rough sample portfolio" : `Signed in as ${user?.email}`}
              </p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 mt-8 flex flex-col gap-8">
        {/* Preset selector (demo only) */}
        {mode === "preset" && (
          <section className="bg-platter border border-amber-500/20 rounded-2xl p-5 shadow-xl flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex items-center gap-2.5">
              <span className="p-1.5 bg-amber-500/10 border border-amber-500/20 text-amber-400 rounded-lg">
                <FlaskConical size={16} />
              </span>
              <div>
                <h2 className="font-sans font-medium text-sm text-white">Pick a sample scenario</h2>
                <p className="text-xs text-white/40">Rough data — no account required</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              {Object.entries(INITIAL_PRESETS).map(([key, p]) => (
                <button
                  key={key}
                  onClick={() => setSelectedPresetKey(key)}
                  className={`px-3.5 py-2 rounded-lg text-[11px] font-mono uppercase tracking-wider border transition cursor-pointer ${
                    selectedPresetKey === key
                      ? "border-amber-400/80 bg-amber-500/10 text-amber-300"
                      : "border-white/5 bg-platter/60 text-white/60 hover:border-white/15"
                  }`}
                >
                  {p.name}
                </button>
        ...
        {loadingPortfolio ? (
          ...
          <EmptyState
            ...
            onCashChange={(v) => {
              setCash(v);
              if (mode === "live") persistAccountProps({ cash: v });
            }}
            onLoanChange={(v) => {
              setLoanBalance(v);
              if (mode === "live") persistAccountProps({ loanBalance: v });
            }}
            onLimitChange={(v) => {
              setMaintenanceLimit(v);
              if (mode === "live") persistAccountProps({ maintenanceLtvLimit: v });
            }}
            isSaving={isSaving}
          />
        ) : (
                ...
                <div className="bg-platter border border-line rounded-2xl p-6 shadow-xl flex flex-col gap-5">
                  <div>
                    ...
                    <p className="text-xs text-white/40 mt-1">
                      {mode === "live" ? "Saved to your portfolio" : "Sample values — edit freely"}
                    </p>
                  </div>
                  <div>
      ...
      {/* Demo simulation toast */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-platter border border-amber-500/30 text-amber-200 rounded-xl px-5 py-3 text-xs font-mono shadow-2xl max-w-md text-center">
          {toast}
          <button onClick={() => setToast(null)} className="ml-3 underline text-amber-400">
            dismiss
          </button>
        </div>
      )}
  ...
  return (
    <div className="max-w-2xl mx-auto bg-platter border border-line rounded-3xl p-8 md:p-10 shadow-xl">
      ...
      <p className="mt-2 text-sm text-white/50 leading-relaxed">
        Enter a position. It&apos;s saved to your account (or simulated locally in demo mode) and drives the optimizer.
      </p>
