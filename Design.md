...
*   **`/src/App.tsx`**: Main UI container for the Elegant Dark dashboard. Orchestrates real-time state for portfolio holdings, active loan balances, market stress test sliders, optimization proposals, and the AI advisor chat desk. Includes model selector with Groq models.
...
*   **`/server.ts`**: Express backend proxying client requests to the Groq API (`api.groq.com/openai/v1`), keeping API keys safe on the server side. Includes Poolside and OpenRouter fallbacks.
...
*   **`/nodes.py`**: Modular Python class implementations for the 6 core nodes:
    ...
    4.  `ReasoningAgentNode`: Synthesizes LTV metrics and ranked lots into a structured recommendation (uses Groq → Poolside → OpenRouter via `init_chat_model` or deterministic fallback). Threads `cash_need` into prompt.
...
The system uses a 3-provider fallback chain with deterministic final fallback:

| Priority | Provider | Model | Base URL | Notes |
|---|---|---|---|---|
| Primary | Groq | `llama-3.3-70b-versatile` | `api.groq.com/openai/v1` | Fast inference, free tier ~14,400 req/day. Replaced Google AI Studio (Gemini direct API). |
| Fallback 1 | Poolside | `poolside/laguna-s-2.1` | `inference.poolside.ai/v1` | Thinking disabled via `{"thinking": {"type": "disabled"}}`. Most reliable for structured output. |
| Fallback 2 | OpenRouter | `google/gemma-4-26b-a4b-it:free` | `openrouter.ai/api/v1` | Free tier ~20 RPM / 200 RPD. Rate limits common. |
| Final | Deterministic | N/A | N/A | Computes safe fallback recommendation without LLM. |

All providers use `with_structured_output(Recommendation, method="function_calling")` for schema-constrained output.
