...
### Added
- **Gemini provider (primary)** (`server.ts`, `nodes.py`, `check_providers.py`): Wired the Gemini Developer API back in as the **primary** LLM provider using the installed `@google/genai` SDK (web) and a direct REST call with `responseSchema` + `responseMimeType: application/json` (Python `ReasoningAgentNode._call_gemini`). Model default `gemini-3-flash-preview` (override via `GEMINI_MODEL`). Key read from `GEMINI_API_KEY` or `GOOGLE_API_KEY`. Satisfies the XPRIZE "≥1 Gemini call in deployed app" rule. Chain is now Gemini → Groq → Poolside → OpenRouter → deterministic.
- **Dynamic model/provider management** (`src/providers.ts`, `server.ts`, `src/components/ModelManager.tsx`, `src/App.tsx`): New `/api/models` endpoints (`GET`, `POST /providers`, `POST /providers/:id/models`, `DELETE`, `POST /default`, `POST /test`). The AI Model Engine card now loads the registry from the server, groups models by provider, and exposes a "Manage Models" panel to add custom OpenAI-compatible providers (any base URL), add models to any provider, live-test connections, set defaults, and delete entries. Custom configs + API keys persist server-side in gitignored `model-config.json`; keys are never sent to the browser.
- **`@google/genai` now actually used** (`server.ts`): `generateViaGemini` via `GoogleGenAI.models.generateContent` replaces the previously unused import.
- **Broken venv fixed**: Recreated `.venv` on CPython 3.11 (was symlinked to a non-existent `/usr/bin/python3.10`).
...
|---|---|---|
| Gemini (`gemini-3-flash-preview`) | Primary | Google Gemini Developer API. Web uses `@google/genai` SDK; Python uses direct REST with `responseSchema`. Key: `GEMINI_API_KEY` or `GOOGLE_API_KEY`. |
| Groq (`llama-3.3-70b-versatile`) | Fallback 1 | Fast inference, free tier ~14,400 req/day. OpenAI-compatible at `api.groq.com/openai/v1`. |
| OpenRouter (`google/gemma-4-26b-a4b-it:free`) | Fallback 3 | Slug verified, quota exhausted | Free-tier ~20 RPM / 200 RPD — daily limit hit during testing. Key works. |
| Poolside (`poolside/laguna-s-2.1`) | Fallback 2 | Thinking disabled via `{"thinking": {"type": "disabled"}}`. `method="function_calling"` confirmed working. All 6 fixtures pass with LLM-generated recommendations. |
