...
def main():
    ...
    for idx, user_acc in enumerate(fake_users, 1):
        ...
        is_skipped = result is not None and isinstance(result, dict) and result.get("status") == "skipped"

        if is_skipped:
            print("  ✓ VERIFIED: Safe portfolio short-circuited via SafeSkipNode as designed.\n")
        else:
            # Fixture-specific assertions only apply when the tax optimizer ran
